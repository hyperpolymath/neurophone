...
image:https://img.shields.io/badge/RSR-Silver%2FGold--track-silver[RSR Compliance]
...
=== Android App (Kotlin — migration in progress)
...
│   ├── java/ai/neurophone/
│   │   ├── MainActivity.kt
│   │   ├── NativeLib.kt          # JNI declarations (Rust impl in crates/neurophone-android)
...
NOTE: The `android/` tree is being migrated to **gossamer** (estate cross-platform shell).
The JNI native implementation has been ported to Rust/Spark (`crates/neurophone-android`).
See RFC PR #97 and epic #83 for the migration roadmap.
...
// Typestate lifecycle: Created → Active → Down
let sys = NeuroSymbolicSystem::new(SystemConfig::default())?
    .initialize()?;                          // → Active (compile error if skipped)
...
sys.process_sensor_event(&event)?;

// Query (prefer local LLM; falls back to Claude API if needed)
let response = sys.query("What's my activity right now?", true)?;

// Inspect state (available on all lifecycle phases)
let state = sys.get_state();

// Terminal shutdown — consumes the system, cannot use after
let _done = sys.shutdown();
```
...
Silver/Gold-track RSR (Rhodium Standard Repository) compliance:

* Type safety (Rust/Spark ownership + typestate lifecycle proofs)
* Memory safety (ownership model — `forbid(unsafe_code)` in core crates)
* Comprehensive proof corpus (`proofs/` — obligations 0.1–3.2 discharged)
* Formal concurrency proofs (`proofs/tla/` — TLA⁺ lifecycle spec)
* Comprehensive documentation and SPDX headers
* Build automation (Justfile, Guix, Nix)
* Security policy (SECURITY.md, hypatia scan, cargo-audit)
* Estate compliance: language-policy, governance, banned-language CI all green
...
Mozilla Public License 2.0 (MPL-2.0) — see link:LICENSE[LICENSE].
