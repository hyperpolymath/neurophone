...
fn bench_iir_lowpass_step(c: &mut Criterion) {
    ...
    c.bench_function("iir_lowpass_step_3ch", |b| {
        b.iter(|| { let _ = black_box(f.step(black_box(&[1.0, 2.0, 3.0]))); })
    });
}
...
fn bench_windowed_features(c: &mut Criterion) {
    ...
    for &cap in &[10usize, 50, 200] {
        ...
        for _ in 0..cap { w.push(&[0.1, 0.2, 0.3]).unwrap(); }
...
fn bench_pipeline_full_second(c: &mut Criterion) {
    c.bench_function("pipeline_50hz_full_second", |b| {
        b.iter(|| {
            let mut p = SensorPipeline::new(SensorKind::Accelerometer, PipelineConfig::default()).unwrap();
            for i in 0..50u64 {
                let r = SensorReading::new(SensorKind::Accelerometer, i * 20, vec![0.1, 0.2, 9.81]).unwrap();
                p.ingest(&r).unwrap();
            }
...
criterion_group!(benches, bench_iir_lowpass_step, bench_iir_highpass_step, bench_windowed_features, bench_pipeline_ingest_50hz, bench_pipeline_full_second);
