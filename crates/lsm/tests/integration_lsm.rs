...
fn small_lsm_constructs_and_steps() {
    let cfg = LsmConfig { dimensions: (4, 4, 4), ..Default::default() };
...
fn lifecycle_step_then_reset() {
    ...
    for _ in 0..50 { let _ = m.step(&Array1::from_vec(vec![0.1, 0.2])); }
    m.reset();
...
fn aspect_input_padding_no_panic() {
    let cfg = LsmConfig { dimensions: (2, 2, 2), ..Default::default() };
    let mut m = LiquidStateMachine::new(cfg, 3).expect("lsm");
...
fn lifecycle_firing_rate_stable_after_warmup() {
    let cfg = LsmConfig { dimensions: (3, 3, 3), ..Default::default() };
    let mut m = LiquidStateMachine::new(cfg, 2).expect("lsm");
    for _ in 0..200 { let _ = m.step(&Array1::from_vec(vec![0.5, 0.5])); }
