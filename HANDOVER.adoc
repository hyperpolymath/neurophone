...
* *LATENT BUILD BREAK — FIXED via PR #129 (MERGED):* workspace `Cargo.toml`
  declared `rand 0.10` / `rand_distr 0.6` (dependabot #49/#67) which are
  incompatible with `ndarray-rand 0.16` (needs `rand 0.9`); only masked by the
  committed `Cargo.lock`. #129 reverted to 0.9/0.5 and is now on `main`, so the
  workspace builds cleanly. (Downstream: #130 carried the same revert — its hunk
  is now redundant and rebases cleanly.)

== 3. My open PRs — recommended merge order

. *#129* `fix/esn-lsm-rand-0.10` — revert `rand`→0.9 / `rand_distr`→0.5;
  un-breaks the workspace build. *MERGED ✓* (was merge-first; everything rebases
  on it).
. *#125* `chore/ignore-agent-worktrees` — gitignore `.claude/worktrees/`; also
  carries this `HANDOVER.adoc`. *Next.*
