...
(`standards/launcher/launcher-standard_praxis.deed` + `docs/UX-standards/launcher-standard.adoc`)
describes a *desktop/server* application launcher. Its load-bearing assumption is
the `(runtime ...)` clause's `:kind "server-url"`: a long-running local process bound to a
...
launcher script. None of the `(runtime ...)` clause fields the standard requires can be
