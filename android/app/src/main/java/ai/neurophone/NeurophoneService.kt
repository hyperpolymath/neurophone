...
class NeurophoneService : Service(), SensorEventListener {
    ...
    override fun onDestroy() {
        ...
        NeurophoneAppWidget.publishState(this, running = false, salience = 0f, description = null)
        super.onDestroy()
    }
    ...
    private fun publishWidgetState() {
        val salience = computeSalience()
        val description = try {
            NativeLib.getNeuralContext()
        } catch (_: Throwable) {
            "[NEURAL_STATE] salience=${"%.2f".format(salience)} (synthetic) [/NEURAL_STATE]"
        }
        NeurophoneAppWidget.publishState(this, running = true, salience = salience, description = description)
    }
